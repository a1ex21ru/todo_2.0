...
func (r *TodoItemPostgres) Create(listId int, item todo.TodoItem) (int, error) {
	...
	row := tx.QueryRow(createItemQuery, item.Title, item.Descripition)
	err = row.Scan(&itemId)
	if err != nil {
		tx.Rollback()
		return 0, err
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todo.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.done FROM %s INNER JOIN %s li on li.item_id = ti.id
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todo.UpdateItemInput) error {
	...
	query := fmt.Sprintf(`UPDATE %s ti SET %s FROM %s li, %s ul WHERE ti.id = li.item_d AND li.list_id = ul.list_id AND ul.user_id = $%d and ti.id = $%d`,
