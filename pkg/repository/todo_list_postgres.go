...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (int, error) {
	...
	_, er := tx.Exec(createUsersListQuery, userId, id)
	if er != nil {
		tx.Rollback()
		return 0, er
	}
...
func (r *TodoListPostgres) GetById(userId, listId int) (todo.TodoList, error) {
	...
	fmt.Printf("list desc:%s, list title: %s", list.Description, list.Title)
	list.Title = "ff"
	list.Description = "fffff"
	return list, err
}

func (r *TodoListPostgres) Delete(userId, listId int) error {
	query := fmt.Sprintf("DELETE FROM %s tl USING %s ul WHERE tl.id = ul.list_id AND ul.user = $1 AND ul.list_id=$2",
		todoListsTable, usersListsTable)
